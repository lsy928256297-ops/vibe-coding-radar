...
Vibe Coding Radar 是一个给 AI Coding 新手看的项目发现网站，也是一枚可以装进 Codex 的项目推荐 Skill：帮你找到值得搓的项目、看清项目亮点、匹配用得上的 Skill / 工具，并复制一段能直接交给 Codex、Cursor 或 Claude Code 的开工提示词。
...
| 调用 | 它会帮你做什么 |
| --- | --- |
| `Find Projects` | 按好玩、好用、硬件、增长最快等方向推荐适合新手的项目。 |
| `Write Starter Prompt` | 把项目来源、GitHub 链接、先做效果和常见卡点整理成可复制给 Codex 的开工提示词。 |
| `Match Skills` | 根据项目本身推荐真正用得上的 Skill / 工具，而不是泛泛贴热门标签。 |
...
你可以把这个目录安装到 Codex 的 Skills 目录，或者让支持 Skill 的代理直接读取这个目录。

## 这个仓库是什么

这是 Vibe Coding Radar 的完整开源项目：网站负责让大家浏览和分享，Skill 负责让 Codex 在对话里直接帮你找项目、找 Skill、写开工提示词。
...
```text
Use Vibe Coding Radar to find 5 fun beginner-friendly Vibe Coding projects.
...
`Vibe Coding Radar` / `Vibe Coding 雷达` 名称、`radar.lyihub.com` 域名和项目视觉识别不包含在 MIT 授权中。你可以 fork、改造和二创，但公开衍生版本请使用不同名称，不要暗示官方关联。详见 [TRADEMARK.md](TRADEMARK.md)。
